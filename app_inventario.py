...
# Caminho do ficheiro de inventário
inventario_path = "inventario_pecas.xlsx"

# Carregar ou criar o ficheiro de inventário
if os.path.exists(inventario_path):
    inventario_df = pd.read_excel(inventario_path, engine="openpyxl")
else:
    inventario_df = pd.DataFrame(columns=["Nome", "Código", "Quantidade", "Criticidade"])

# Carregar a base de peças críticas
criticas_df = pd.read_excel("base_pecas_criticas.xlsx", engine="openpyxl")

# Interface Streamlit
st.title("Inventário de Peças - Armazém DPD Lisbon")
...
# Verificar peças críticas com quantidade zero ou baixa
comparacao_df = criticas_df.merge(inventario_df, on="Código", how="left")
...
st.dataframe(pecas_em_falta[["Nome_x", "Código", "Quantidade"]])

st.subheader("🔵 Peças Críticas com Stock Baixo")
st.dataframe(pecas_baixo_stock[["Nome_x", "Código", "Quantidade"]])
