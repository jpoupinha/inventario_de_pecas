...
pecas_em_falta = comparacao_df[(comparacao_df["Criticidade"] == 1) & (comparacao_df["Quantidade"] < 1)]
pecas_baixo_stock = comparacao_df[(comparacao_df["Criticidade"] == 1) & (comparacao_df["Quantidade"] > 0) & (comparacao_df["Quantidade"] < 2)]

st.subheader("⚠️ Peças Críticas em Falta")
st.dataframe(pecas_em_falta[["Nome", "Item", "Quantidade"]])

st.subheader("🔵 Peças Críticas com Stock Baixo")
st.dataframe(pecas_baixo_stock[["Nome", "Item", "Quantidade"]])
